...
| `verify_logs_listener_skips_failed_tx.py` | a `logsNotification` carrying an `err` yields no coin, so a create whose transaction reverted is not bought; a successful one is still detected, and geyser still excludes failed transactions server-side |
| `verify_creator_migration_from_logs.py` | a fee-sharing coin's `creator` comes from `MigrateBondingCurveCreatorEvent` in the same logs, not from `CreateEvent.creator`, so `creator_vault` matches the curve and the buy does not revert `ConstraintSeeds`; ordinary creates unchanged and `state_from_event` kept |
