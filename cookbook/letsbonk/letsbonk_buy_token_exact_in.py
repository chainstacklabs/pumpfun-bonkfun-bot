"""Buy a letsbonk.fun coin with buy_exact_in, spending a fixed amount of SOL.

WARNING: this submits a real transaction and spends real funds.

Usage:
    uv run cookbook/letsbonk/letsbonk_buy_token_exact_in.py <MINT> [SOL_TO_SPEND] [--slippage 0.25]
...
async def buy_exact_in(
    ...
    `buy_exact_in` takes 18 accounts: the 15 the IDL lists, plus three remaining
    accounts — System Program, creator fee vault, platform fee vault.
...
async def run(token_mint: Pubkey, amount: float, slippage: float) -> None:
    """Execute the buy_exact_in example.
    ...
    """
    try:
        print(f"Starting buy_exact_in for token: {token_mint}")
        print(f"Amount to spend: {amount} SOL")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Buy a letsbonk.fun coin by spending a fixed amount of SOL")
    ...
    parser.add_argument(
        ...
        help=f"SOL to spend (default {DEFAULT_AMOUNT})",
    )
    parser.add_argument(
        "--slippage",
        type=float,
