"""Sell a letsbonk.fun coin with sell_exact_in, selling a fixed token amount.

WARNING: this submits a real transaction and spends real funds.

Usage:
    uv run cookbook/letsbonk/letsbonk_sell_token_exact_in.py <MINT> [TOKENS_TO_SELL] [--slippage 0.25]

letsbonk.fun runs on Raydium LaunchLab, which quotes in two directions:
*exact in* fixes what you sell, *exact out* fixes what you receive. Slippage
protection is `minimum_amount_out`. WSOL is wrapped and unwrapped around the
trade.
"""
...
async def sell_exact_in(
    ...
    Instruction order: compute unit price and limit, WSOL account creation and
    initialization, the sell, then closing WSOL.
...
async def run(token_mint: Pubkey, amount: int, slippage: float) -> None:
    """Execute the sell_exact_in example.

    Args:
        token_mint: The coin to trade
        amount: Whole tokens to sell
        slippage: Slippage tolerance
    """
    try:
        print(f"Starting sell_exact_in for token: {token_mint}")
        print(f"Amount to sell: {amount:,} tokens")
            ...
            try:
                ...
                if token_account_info.value:
                    ...
                    if token_balance < amount:
                        print(
                            f"Insufficient token balance! You have {token_balance:,} tokens but want to sell {amount:,}"
                        )
                        return
                else:
